...
const Listings = () => {
  ...
  return (
    <div className="relative px-28  max-sm:px-1 ">
      ...
      <div className="relative w-full flex flex-col p-2  max-sm:h-screen max-sm:w-full  h-screen   rounded-tl-lg pb-2">
        <div className="relative h-[24rem] max-sm:w-full  bg-gray-100 rounded-lg  flex max-sm:flex-col gap-2 justify-start  mt-4 items-start">
          {userDeals.map(
            ({ budget, jobTitle, deadline, jobExecutionState, id }) => (
              <Job
                id={id}
                budget={budget}
                key={id}
                jobTitle={jobTitle}
                deadline={deadline}
                executionStatus={jobExecutionState}
              />
            )
          )}
        </div>
