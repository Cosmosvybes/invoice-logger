            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              {!isPro ? (
                 <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest flex items-center gap-1">
                    <Lock className="text-xs" /> PRO FEATURE
                 </span>
              ) : (
                <>
      ...
      <div className="relative flex-1 flex flex-col overflow-hidden">
          {!isPro && (
              <div className="absolute inset-0 z-50 bg-white/60 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-700">
                  <div className="w-20 h-20 bg-violet-100 rounded-full flex items-center justify-center mb-6 text-violet-600 shadow-xl shadow-violet-100">
                      <Lock className="text-4xl" />
                  </div>
                  <h2 className="text-3xl font-black text-slate-900 mb-3 tracking-tight">Unlock Invoice Chat</h2>
                  <p className="text-slate-500 max-w-md font-medium mb-8">Collaborate with clients in real-time context directly on your invoices. Upgrade to Pro to access this feature.</p>
                  <button 
          ...
          <main className={`flex-1 overflow-y-auto no-scrollbar bg-[radial-gradient(circle_at_top_right,rgba(124,58,237,0.03),transparent)] px-6 md:px-12 ${!isPro ? 'blur-sm select-none pointer-events-none' : ''}`}>
          ...
          <footer className={`shrink-0 p-6 md:p-10 bg-gradient-to-t from-slate-50 to-transparent ${!isPro ? 'blur-sm select-none pointer-events-none' : ''}`}>
